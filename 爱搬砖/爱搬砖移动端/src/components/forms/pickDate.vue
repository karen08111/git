<template>
	<div class="select">
		...
		<van-popup v-model="show" click-overlay="shows" position="bottom" >
			<van-datetime-picker v-model="currentDate" type="year-month"  @confirm="confirm" @cancel="cancel" :min-date="minDate" :formatter="formatter"/>
		</van-popup>
...
<script>
	export default {
		...
		mounted() {
			var now = new Date;
		    now.setYear(now.getFullYear()-50);
		    this.minDate =now
		},
		methods: {
			shows() {
			...
			confirm(e) {
				var mon = parseInt(e.getMonth()+1) 
				if(mon < 10) {
					mon = '0' + mon	
				}
				this.city = e.getFullYear()+'-'+mon
				this.$emit("giveDate",this.city)
				this.show = false;
				this.isShade = false;
