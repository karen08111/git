<template>
	<div class="addBasic">
		<div class="form">
			...
			<div class="import">
				<span class="import-title">工种</span>
				<JobSelect @giveId="giveId" :cityString="workName"></JobSelect>
			</div>
			<div class="import">
			...
			<div class="import">
				<span class="import-title letter-none">参加工作时间</span>
...
//	import SimpleCropper from '@/components/froms/SimpleCropper'
	export default {
		...
		mounted() {
			this.$http.get("/member/resume/moveEdit").then(res=>{
				if(res.code == 200) {
...
//					console.log(res.data.detail.name)
					...
					this.userInfo = res.data.detail;
					this.avatar =res.data.detail.avatar;
					this.mobile= detail.mobile;
					this.wechat_number = detail.wechat_number;
					this.current_state = detail.current_state;
					if(detail.region.length == 3) this.region = detail.region[1].id;
					if(detail.region.length == 3) this.region = detail.region[2].id;
					this.checkout = detail.checkout
					this.has_bad_medical_history = detail.has_bad_medical_history;
					this.min_pay = detail.min_pay;
					this.name=detail.name;
					this.quantity_unit = detail.quantity_unit.id
					this.resume_type = detail.resume_type
					if(detail.work_time) this.work_time = detail.work_time[1]
					if(detail.region.length>0) this.regionName = detail.region.map(value=>value.title).join("-")
					if(detail.work_type) this.work_type = detail.work_type.id;
					if(detail.work_type) this.jobTwo = detail.work_type.title
					
//					console.log(this.jobTwo)
					//占位
					this.unit = detail.quantity_unit.unit
					this.lay = detail.min_pay
				}
...
//								if(this.work_type == res.data[i].children[j].id ){
//	//								this.str = res.data[i].work_name+'-'+res.data[i].children[j].work_name
//									console.log(res.data[i].work_name)
//									break;
//								}
//							}
//						}
...
//					return name.title
		...
		methods: {
			onRead(file) {
			    ...
			    this.$http.post("/member/resume/avatar",data).then(res=>{
			    	if(res.code == 200) {
			...
			save() {
				var data = {
					...
					has_bad_medical_history:this.has_bad_medical_history,
				...
				var that = this
				this.$http.post("/member/resume/moveSave",data).then(res=>{
					if(res.code == 200) {
