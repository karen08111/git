	...
	<div class="main">
		<div class="bai">
			<div class="shang">
				<div class="info-head">
					<div class="info-left">
						<p class="info-title" v-if="info.title">{{info.title}}</p>
					...
					<div class="info-avatar">
						<p>{{info.min_pay==0?'面议':info.min_pay+'/'+info.quantity_unit}}</p>
						<img :src="info.avatar?baseURL+info.avatar:'../../assets/none.png'"/>
					</div>
					<div class="clear"></div>
				</div>
				<div class="contents">
					<p class="content-title">开工时间:{{info.start_time}}</p>
					<p class="content-main" v-if="info.work_time">工作年限:{{info.work_time}}</p>
				</div>
			</div>
			<div class="content">
				<p class="job-list-title"><span></span>招聘描述</p>
				<p class="job-content">{{info.content}}</p>
			</div>
			<p class="job-list-title top"><span></span>推荐招聘工人信息</p>
		</div>
		<!--推荐信息-->
...
//			console.log()
		...
		methods: {
			getRequest() {
				var that = this
				this.$http.get("/recruit/detail/"+this.id).then(res=>{
					if(res.code==200){
//						console.log(res.data)
          ...
          that.fenxiang()
			...
			guanzhu(){
				var that = this
				that.$http.get("/member/inReleaseFollow?follow_id="+this.id+"&follow_type=3").then(res=>{
					if(res.code==200){
						this.isReset=1
					}else {
						that.isshows = true
...
//						},1500)
			...
			quguan(){
				var that = this
				that.$http.get("/member/offReleaseFollow?follow_id="+this.id+"&follow_type=3").then(res=>{
					if(res.code==200){
						this.isReset=0
					}
				})
			...
			lianxi() {
				var that = this
				that.$http.get("/member/getInfoMobile/"+that.id+'/3').then(res=>{
					if(res.code==200){
						that.isshows = true
