<template>
	<div class="main">
		<div class="user-info">
			<div class="user-left">
				<div class="user-name">
					...
					<img v-if="info.sex=='男'" src="../../assets/none.png"/>
				...
				<div class="user-job-list">
					...
					<span class="border-right border-left">平均工龄:{{info.avg_work_time}}年</span>
					<span class="border-right border-left">平均年龄:{{info.avg_age}}</span>
					<span class="border-right" v-if="info.team_create_time">{{info.team_create_time}}</span>
				</div>
		...
		<!--工作经历-->
		<div class="job-list">
			<p class="job-list-title"><span></span>工程业绩</p>
			<!--xunhuan-->
			...
			<div class="job-li" v-if="!job_caseLength">
				<p class="job-content">暂无信息</p>
			</div>
		</div>
		<!--资质证书-->
		...
		<!--个人介绍-->
		<div class="content">
			<p class="job-list-title"><span></span>团队介绍</p>
		...
		<div class="zhezhao" v-if="isshows"></div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			getRequest() {
				var that = this
				that.$http.get("/team/detail/"+that.id).then(res=>{
//					console.log(res.data)
					if(res.code==200){
						...
						this.job_caseLength = res.data.job_case.length
          ...
          that.fenxiang()
			...
			guanzhu(){
				var that = this
				that.$http.get("/member/inReleaseFollow?follow_id="+this.id+"&follow_type=2").then(res=>{
					if(res.code==200){
						this.isReset=1
			...
			quguan(){
				var that = this
				that.$http.get("/member/offReleaseFollow?follow_id="+this.id+"&follow_type=2").then(res=>{
					if(res.code==200){
						this.isReset=0
					}
				})
			...
			lianxi(){
				var that = this
				that.$http.get("/member/getInfoMobile/"+that.id+'/2').then(res=>{
					if(res.code==200){
						that.isshows = true
				...
				this.$http.post('/common/third_auth/share_sign',formData).then(res=>{
			        ...
			        wx.ready(function () {
						...
						wx.onMenuShareAppMessage({
							title: that.info.team_name, // 分享标题
