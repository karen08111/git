<template>
  <div class="login">
    <h1>登 录</h1>
    <i class="el-icon-error" @click="$store.commit('changeLogin')"></i>
    <el-form :model="ruleForm2" status-icon :rules="rules2" ref="ruleForm2" label-width="0" class="demo-ruleForm">
      ...
      <el-form-item label="" prop="imageCode" style="margin-bottom: 30px; padding-left: 160px; position:relative;">
        ...
        <div class="check_img" @click="changeUrl"><img :src="url" :onerror="$store.state.defaultImg"></div>
      </el-form-item>
      ...
      <el-form-item style="padding-left: 160px; margin-bottom: 12px">
        <el-button type="primary" @click="submitForm('ruleForm2')" style="width: 320px;">登 录</el-button>
      </el-form-item>
    </el-form>
    <div class="other">没有账号？<span @click="$store.commit('regis')">立即注册</span></div>
    <!-- <div class="third">
...
export default {
  ...
  methods: {
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.allow) {
            this.$http.post(this.$store.state.site + '/member/login/mobile', formData, {
              headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).then(res => {
              if (res.data.code == 200) {
                this.$cookies.set('abztoken', res.data.data.token, '0')
                if (res.data.data.status == 0) {
                  this.$cookies.set('usertel', res.data.data.mobile, '0')
                } else {
                  this.$cookies.set('auth', res.data.data.dq_type, '0')
                  this.$cookies.set('usertel', res.data.data.mobile, '0')
                  this.$cookies.set('username', res.data.data.name, '0')

                  if (res.data.data.status == 1 || res.data.data.status == 2) {
                    this.$cookies.set('authState', 1, '0')
                  } else if (res.data.data.status == 4) {
                    this.$cookies.set('authState', 2, '0')
                  } else if (res.data.data.status == 3) {
                    this.$cookies.set('authState', 3, '0')
                  }
                }
    ...
    getAuthCode: function () {
      ...
      this.$http.post(this.$store.state.site + '/member/captcha/vermob', formData, {
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
      }).then(res => {
        if (res.data.code == 200) {
          this.allow = true
    ...
    changeUrl () {
      this.$http.get(this.$store.state.site + '/member/captcha/verimg').then(res => {
        this.url = res.data
