<template>
  <div class="container">
    <header><router-link :to="`/Recruit_form/2?tid=${$route.params.id}`"><el-button type="primary" plain>发布招聘信息</el-button></router-link></header>
    <div class="list">
      <ul>
...
export default {
  ...
  methods: {
    edit (id) {
      this.$router.push({ name: 'Recruit_form', params: { id: id, publisher: 2 } })
    },
    hidden (is_show, id) {
      let methods = ''
    ...
    request () {
      this.$http.get('/member/recruit', { params: { page: this.page, publisher: 2, tid: this.$route.params.id } }).then(res => {
        if (res.data.code == 200) {
